...
struct EXTERNAL ISystemMode : virtual public Core::IUnknown {
  ...
  enum State : uint32_t {
	  ...
	  VIDEO = (DEVICE_OPTIMIZE << 24) | 1 ,
	  GAME  = (DEVICE_OPTIMIZE << 24) | 2 ,
  ...
  virtual Core::hresult RequestState(const string& systemMode, const string& state) = 0;
  ...
  // @returns Core::hresult
  virtual Core::hresult GetState(const string& systemMode, string &state /* @out */) const = 0;
